...
#define __NR_syscall 41	/* socket()系统调用号41 */
unsigned long * sys_call_table;
unsigned long * orig_socket = NULL;

unsigned int clear_and_return_cr0(void);
void setback_cr0(unsigned int val);
// static int sys_mycall(void);
int hack_socket( int af, int type, int protocol);

int orig_cr0;	/* 用来存储cr0寄存器原来的值 */
unsigned long *sys_call_table = 0;
static int (*anything_saved)(int af, int type, int protocol);	/*定义一个函数指针，用来保存一个系统调用*/
/*
 * 设置cr0寄存器的第17位为0
 */
unsigned int clear_and_return_cr0(void)	
...
int hack_socket( int af, int type, int protocol)
{
	printk("maybe you want to socket()\n");
	return(*anything_saved)(af, type, protocol);
}
...
static int __init init_addsyscall(void)
{
	...
	anything_saved = (int(*)(int af, int type, int protocol))(sys_call_table[__NR_syscall]);	/* 保存原始系统调用 */
	orig_cr0 = clear_and_return_cr0();	/* 设置cr0可更改 */
	sys_call_table[__NR_syscall] = (unsigned long)&hack_socket;	/* 更改原始的系统调用服务地址 */
